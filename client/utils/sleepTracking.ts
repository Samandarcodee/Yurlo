...
// Update sleep goals
export const updateSleepGoals = (
  telegramId: string,
  goals: Partial<SleepGoals>,
): SleepGoals => {
...
export const updateSleepSession = (
  ...
  if (sleepSession.bedTime && sleepSession.wakeTime) {
    sleepSession.sleepDuration = calculateSleepDuration(
...
export const getSleepInsights = (telegramId: string): SleepInsights => {
  ...
  const averageDuration =
    history.reduce((sum, s) => sum + s.sleepDuration, 0) / history.length;
  const averageQuality =
    history.reduce((sum, s) => sum + s.sleepQuality, 0) / history.length;
  const averageFallAsleepTime =
    history.reduce((sum, s) => sum + s.fellAsleepTime, 0) / history.length;
  ...
  const consistencyScore =
    maxPoints > 0 ? Math.round((consistencyPoints / maxPoints) * 100) : 0;
  ...
  const weeklyPattern = dayNames.map((day) => {
    ...
    const avgBedMinutes =
      daySessions.reduce((sum, s) => sum + timeToMinutes(s.bedTime), 0) /
      daySessions.length;
    const avgWakeMinutes =
      daySessions.reduce((sum, s) => sum + timeToMinutes(s.wakeTime), 0) /
      daySessions.length;
    const avgDuration =
      daySessions.reduce((sum, s) => sum + s.sleepDuration, 0) /
      daySessions.length;
    const avgQuality =
      daySessions.reduce((sum, s) => sum + s.sleepQuality, 0) /
      daySessions.length;
  ...
  if (averageQuality < 3) {
    recommendations.push(
      "Uyqu sifatini yaxshilash uchun uyqu muhitini optimallashtiring",
    );
  }

  if (averageFallAsleepTime > 30) {
    recommendations.push(
      "Uxlashdan oldin meditatsiya yoki kitob o'qing - tez uxlash uchun",
    );
  }

  if (consistencyScore < 70) {
    recommendations.push(
      "Uyqu jadvalingizni barqarorlashtiring - har kuni bir xil vaqtda yoting va turing",
    );
  }

  const recentQuality =
    last7Days.length > 0
      ? last7Days.reduce((sum, s) => sum + s.sleepQuality, 0) / last7Days.length
      : 0;
  if (recentQuality > averageQuality) {
    recommendations.push(
      "Yaxshi! Uyqu sifatingiz yaxshilanmoqda. Davom eting!",
    );
  }

  if (recommendations.length === 0) {
    recommendations.push(
      "Uyqu sifatingiz yaxshi! Joriy jadvalingizni davom ettiring.",
    );
  }
  ...
  const firstHalfDuration =
    firstHalf.reduce((sum, s) => sum + s.sleepDuration, 0) / firstHalf.length ||
    0;
  const secondHalfDuration =
    secondHalf.reduce((sum, s) => sum + s.sleepDuration, 0) /
      secondHalf.length || 0;

  const firstHalfQuality =
    firstHalf.reduce((sum, s) => sum + s.sleepQuality, 0) / firstHalf.length ||
    0;
  const secondHalfQuality =
    secondHalf.reduce((sum, s) => sum + s.sleepQuality, 0) /
      secondHalf.length || 0;

  const durationTrend =
    secondHalfDuration > firstHalfDuration + 0.2
      ? "improving"
      : secondHalfDuration < firstHalfDuration - 0.2
        ? "declining"
        : "stable";
  ...
  const qualityTrend =
    secondHalfQuality > firstHalfQuality + 0.3
      ? "improving"
      : secondHalfQuality < firstHalfQuality - 0.3
        ? "declining"
        : "stable";
...
export const calculateSleepScore = (
  ...
  const bedTimeDiff = Math.abs(
    timeToMinutes(session.bedTime) - timeToMinutes(goals.targetBedTime),
  );
