import { getSleepGoals, getTodaySleep, isBedtimeReminder } from './sleepTracking';
...
export const getSleepNotifications = (telegramId: string): SleepNotification[] => {
...
export const markNotificationAsRead = (telegramId: string, notificationId: string): void => {
  const notifications = getSleepNotifications(telegramId);
...
export const checkSleepNotifications = (telegramId: string): SleepNotification[] => {
  const goals = getSleepGoals(telegramId);
  const todaySleep = getTodaySleep(telegramId);
  const now = new Date();
  const currentTime = `${now.getHours().toString().padStart(2, '0')}:${now.getMinutes().toString().padStart(2, '0')}`;
  
  const newNotifications: Omit<SleepNotification, 'id' | 'timestamp' | 'isRead'>[] = [];
  ...
  const targetWakeMinutes = parseInt(goals.targetWakeTime.split(':')[0]) * 60 + parseInt(goals.targetWakeTime.split(':')[1]);
  ...
  if (!hasSleepData && now.getHours() === 10) { // 10 AM reminder
    newNotifications.push({
      type: 'reminder',
      title: '📊 Uyqu tracking eslatmasi',
      message: 'Uyqu ma\'lumotlaringizni kuzatib boring! Yaxshi uyqu sog\'ligingiz kaliti.',
      priority: 'low',
      actionButton: {
        text: 'Uyqu tracker',
  ...
  if (now.getDay() === 0 && now.getHours() === 9) { // Sunday 9 AM
    newNotifications.push({
      ...
      message: 'O\'tgan hafta uyqu statistikangizni ko\'ring va kelgusi hafta uchun maqsad qo\'ying.',
      priority: 'medium',
      actionButton: {
...
export const getSleepOptimizationTips = (
  ...
  if (averageDuration < 7) {
    tips.push("⏰ Uyqu vaqtingizni oshiring - kamida 7-8 soat uxlashga harakat qiling");
    tips.push("🕒 Ertaroq yotishga harakat qiling va telefon vaqtini cheklang");
  }
  
  if (averageQuality < 3.5) {
    tips.push("🌡️ Uyqu muhitini optimallashtiring - harorat, yoruglik va shovqin");
  ...
  if (consistencyScore < 70) {
    tips.push("⏰ Har kuni bir xil vaqtda yoting va turing - dam olish kunlarida ham");
...
export const getSleepScoreBadge = (score: number): { text: string; color: string } => {
  if (score >= 80) return { text: 'A\'lo', color: 'bg-green-100 text-green-800' };
  if (score >= 60) return { text: 'Yaxshi', color: 'bg-yellow-100 text-yellow-800' };
  if (score >= 40) return { text: 'O\'rtacha', color: 'bg-orange-100 text-orange-800' };
