...
export const calculateDistance = (steps: number, userHeight: number = 170): number => {
...
export const calculateCaloriesFromSteps = (steps: number, weight: number = 70): number => {
...
// Update step goals
export const updateStepGoals = (telegramId: string, goals: Partial<StepGoals>): StepGoals => {
...
export const getStepHistory = (telegramId: string, days: number = 30): StepSession[] => {
...
export const getStepInsights = (telegramId: string): StepInsights => {
  ...
  // Calculate averages
  const dailyAverage = Math.round(history.reduce((sum, s) => sum + s.steps, 0) / history.length);
  const weeklyAverage = last7Days.length > 0 
    ? Math.round(last7Days.reduce((sum, s) => sum + s.steps, 0) / last7Days.length)
    : dailyAverage;
  ...
  const firstHalfAvg = firstHalf.reduce((sum, s) => sum + s.steps, 0) / firstHalf.length || 0;
  const secondHalfAvg = secondHalf.reduce((sum, s) => sum + s.steps, 0) / secondHalf.length || 0;
  
  const stepsChange = secondHalfAvg - firstHalfAvg;
  const stepsChangePercent = firstHalfAvg > 0 ? (stepsChange / firstHalfAvg) * 100 : 0;
  
  const stepstrend = stepsChangePercent > 5 ? 'increasing' 
                   : stepsChangePercent < -5 ? 'decreasing' 
                   : 'stable';
  
  // Similar for distance
  const firstHalfDistance = firstHalf.reduce((sum, s) => sum + s.distance, 0) / firstHalf.length || 0;
  const secondHalfDistance = secondHalf.reduce((sum, s) => sum + s.distance, 0) / secondHalf.length || 0;
  const distanceChange = secondHalfDistance - firstHalfDistance;
  const distanceChangePercent = firstHalfDistance > 0 ? (distanceChange / firstHalfDistance) * 100 : 0;
  
  const distanceTrend = distanceChangePercent > 5 ? 'increasing'
                      : distanceChangePercent < -5 ? 'decreasing'
                      : 'stable';
  
  // Consistency score
  const daysWithGoalMet = history.filter(s => s.steps >= goals.dailySteps).length;
  ...
  history.forEach(session => {
    ...
    weeklyPattern[dayName] = (weeklyPattern[dayName] + session.steps) || session.steps;
  });
  
  // Average out the weekly pattern
  const weekCounts = { monday: 0, tuesday: 0, wednesday: 0, thursday: 0, friday: 0, saturday: 0, sunday: 0 };
  history.forEach(session => {
    const dayOfWeek = new Date(session.date).getDay();
  ...
  Object.keys(weeklyPattern).forEach(day => {
    ...
    if (weekCounts[dayKey] > 0) {
      weeklyPattern[dayKey] = Math.round(weeklyPattern[dayKey] / weekCounts[dayKey]);
    }
  });
  ...
  if (dailyAverage < goals.dailySteps) {
    ...
    recommendations.push(`Kunlik maqsadga erishish uchun yana ${deficit.toLocaleString()} qadam tashlang`);
  }
  ...
  if (currentStreak === 0) {
    ...
    recommendations.push(`Ajoyib! ${currentStreak} kunlik streak'ni davom ettiring`);
  }
  
  const leastActiveDay = Object.entries(weeklyPattern).reduce((min, [day, steps]) =>
    steps < min.steps ? { day, steps } : min,
    { day: 'monday', steps: Infinity }
  );
  
  if (leastActiveDay.steps < dailyAverage * 0.8) {
    recommendations.push(`${leastActiveDay.day.charAt(0).toUpperCase() + leastActiveDay.day.slice(1)} kunlari ko'proq faol bo'lishga harakat qiling`);
  }
  
  if (stepstrend === 'decreasing') {
    recommendations.push('Faolligingiz kamaymoqda - yangi motivatsiya topishga harakat qiling');
  }
  
  // Health metrics calculation
  const cardiovascularHealth = Math.min(100, Math.round((dailyAverage / goals.dailySteps) * 100));
  const fitnessLevel = Math.min(100, Math.round(((dailyAverage / 10000) * 60 + (consistencyScore * 0.4))));
  const weightManagement = Math.min(100, Math.round(((dailyAverage / 10000) * 50 + (consistencyScore * 0.5))));
    ...
    switch (achievement.requirement.type) {
      ...
      case 'single_day':
        if (achievement.category === 'steps') {
          achieved = history.some(s => s.steps >= achievement.requirement.value);
        } else if (achievement.category === 'distance') {
          achieved = history.some(s => s.distance * 1000 >= achievement.requirement.value); // convert km to m
        }
        break;
...
export const simulateStepCounting = (telegramId: string): void => {
  const interval = setInterval(() => {
    ...
    if (current.steps < 25000) { // Don't go too crazy
...
export const addSampleStepData = (telegramId: string, userHeight: number = 170, userWeight: number = 70): void => {
...
// Get step challenges
export const getActiveStepChallenges = (telegramId: string): StepChallenge[] => {
  const storageKey = `stepChallenges_${telegramId}`;
  const challenges: StepChallenge[] = JSON.parse(localStorage.getItem(storageKey) || '[]');
