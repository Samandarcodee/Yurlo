...
export const addWaterEntry = (
  ...
  const totalIntake = updatedEntries.reduce(
    (sum, entry) => sum + entry.amount,
    0,
  );
  const totalVolume = updatedEntries.reduce(
...
// Update water goals
export const updateWaterGoals = (
  telegramId: string,
  goals: Partial<WaterGoals>,
): WaterGoals => {
...
export const getWaterInsights = (telegramId: string): WaterInsights => {
  ...
  // Calculate averages
  const dailyAverage =
    history.reduce((sum, s) => sum + s.totalIntake, 0) / history.length;
  const weeklyAverage =
    last7Days.length > 0
  ...
  const bestDay = history.reduce(
    (best, current) =>
      current.totalIntake > best.glasses
        ? { date: current.date, glasses: current.totalIntake }
        : best,
    { date: "", glasses: 0 },
  );
  ...
  const morningEntries = history.flatMap((s) =>
    s.entries.filter((e) => {
  ...
  const afternoonEntries = history.flatMap((s) =>
    s.entries.filter((e) => {
      const hour = new Date(e.timestamp).getHours();
  ...
  const eveningEntries = history.flatMap((s) =>
    s.entries.filter((e) => {
      const hour = new Date(e.timestamp).getHours();
  ...
  const morningIntake =
    morningEntries.reduce((sum, e) => sum + e.amount, 0) / history.length;
  const afternoonIntake =
    afternoonEntries.reduce((sum, e) => sum + e.amount, 0) / history.length;
  const eveningIntake =
    eveningEntries.reduce((sum, e) => sum + e.amount, 0) / history.length;
  ...
  const excellentDays = history.filter(
    (s) => s.totalIntake >= s.goal * 1.2,
  ).length;
  const goodDays = history.filter(
    (s) => s.totalIntake >= s.goal && s.totalIntake < s.goal * 1.2,
  ).length;
  const moderateDays = history.filter(
    (s) => s.totalIntake >= s.goal * 0.7 && s.totalIntake < s.goal,
  ).length;
  ...
  const firstHalfAvg =
    firstHalf.reduce((sum, s) => sum + s.totalIntake, 0) / firstHalf.length ||
    0;
  const secondHalfAvg =
    secondHalf.reduce((sum, s) => sum + s.totalIntake, 0) / secondHalf.length ||
    0;

  const intakeChange = secondHalfAvg - firstHalfAvg;
  const intakeChangePercent =
    firstHalfAvg > 0 ? (intakeChange / firstHalfAvg) * 100 : 0;

  const intakeTrend =
    intakeChangePercent > 10
      ? "increasing"
      : intakeChangePercent < -10
        ? "decreasing"
        : "stable";
  ...
  if (dailyAverage < 6) {
    recommendations.push(
      "Suv ichish miqdorini oshiring - kamida 6-8 stakan suv ichi",
    );
  }

  if (morningIntake < 2) {
    recommendations.push(
      "Ertalab ko'proq suv iching - uyg'ongandan keyin 1-2 stakan",
    );
  }
  ...
  const energyLevel = Math.min(
    100,
    Math.round((dailyAverage / 8) * 80 + consistencyScore * 0.2),
  );
  const detoxification = Math.min(100, Math.round((dailyAverage / 10) * 100));
  const weightManagement = Math.min(
    100,
    Math.round((dailyAverage / 8) * 60 + consistencyScore * 0.4),
  );
...
export const addSampleWaterData = (
  telegramId: string,
  userGoal: number = 8,
): void => {
      ...
      for (let j = 0; j < 3 && currentIntake < glasses; j++) {
        ...
        if (adjustedAmount > 0) {
          entries.push({
            ...
            timestamp: `${dateKey}T${(8 + j * 2).toString().padStart(2, "0")}:${Math.floor(
              Math.random() * 60,
            )
              .toString()
              .padStart(2, "0")}:00Z`,
      ...
      while (currentIntake < glasses && entries.length < 8) {
        ...
        if (adjustedAmount > 0) {
          ...
          entries.push({
            ...
            amount: Math.round(adjustedAmount * 4) / 4,
            volume: glassesToMl(adjustedAmount),
            timestamp: `${dateKey}T${hour.toString().padStart(2, "0")}:${Math.floor(
              Math.random() * 60,
            )
              .toString()
              .padStart(2, "0")}:00Z`,
...
export const checkWaterReminders = (telegramId: string): WaterReminder[] => {
  ...
  const storageKey = `waterReminders_${telegramId}`;
  const reminders: WaterReminder[] = JSON.parse(
    localStorage.getItem(storageKey) || "[]",
  );

  return reminders.filter(
    (reminder) => reminder.isActive && reminder.time === currentTime,
  );
};
