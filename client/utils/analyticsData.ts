...
export const getCaloriesChartData = (telegramId: string, days: number = 30): TrendData => {
  ...
  for (let i = days - 1; i >= 0; i--) {
    ...
    data.push({
      date: dateKey,
      value: calories,
      label: date.toLocaleDateString('uz-UZ', { day: '2-digit', month: 'short' })
    });
  }
...
export const getWeightChartData = (telegramId: string, days: number = 90): TrendData => {
  const data: ChartDataPoint[] = [];
  ...
  const trend = changePercentage > 1 ? 'increasing' : changePercentage < -1 ? 'decreasing' : 'stable';
...
export const getStepsChartData = (telegramId: string, days: number = 30): TrendData => {
  const data: ChartDataPoint[] = [];
  ...
  const trend = changePercentage > 10 ? 'increasing' : changePercentage < -10 ? 'decreasing' : 'stable';
...
export const getSleepChartData = (telegramId: string, days: number = 30): TrendData => {
  const data: ChartDataPoint[] = [];
  ...
  const trend = average > target ? 'increasing' : average < target - 0.5 ? 'decreasing' : 'stable';
...
export const getNutritionAnalytics = (telegramId: string): NutritionAnalytics => {
  const weeklyStats = getWeeklyStats(telegramId);
  const caloriesData = getCaloriesChartData(telegramId, 7);
  
  const averageCalories = caloriesData.data.reduce((sum, d) => sum + d.value, 0) / 7;
...
export const getBodyCompositionData = (telegramId: string, user: UserProfile): BodyComposition[] => {
...
export const calculateHealthScore = (telegramId: string): { score: number; factors: { name: string; score: number; weight: number }[] } => {
