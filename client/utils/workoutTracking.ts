...
export const calculateWorkoutCalories = (
  ...
  const met =
    metValues[type]?.[intensity] || metValues.cardio[intensity] || 6.0;
...
export const updateWorkoutGoals = (
  telegramId: string,
  goals: Partial<WorkoutGoals>,
): WorkoutGoals => {
...
export const getWorkoutInsights = (telegramId: string): WorkoutInsights => {
  ...
  const weeklyStats = {
    ...
    averageDuration:
      weekHistory.length > 0
        ? weekHistory.reduce((sum, w) => sum + w.duration, 0) /
          weekHistory.length
        : 0,
  ...
  const consistencyScore = Math.round(
    (weekHistory.length / goals.weeklyWorkouts) * 100,
  );

  // Patterns
  const workoutTypes = history.map((w) => w.type);
  const favoriteWorkoutTypes = [...new Set(workoutTypes)]
    .sort(
      (a, b) =>
        workoutTypes.filter((t) => t === b).length -
        workoutTypes.filter((t) => t === a).length,
    )
    .slice(0, 3) as WorkoutType[];

  const equipment = history.flatMap((w) => w.equipment);
  const mostUsedEquipment = [...new Set(equipment)]
    .sort(
      (a, b) =>
        equipment.filter((e) => e === b).length -
        equipment.filter((e) => e === a).length,
    )
    .slice(0, 5);
  ...
  const workoutFrequency =
    freqChange > 1 ? "increasing" : freqChange < -1 ? "decreasing" : "stable";
  ...
  if (weeklyStats.averageDuration < goals.sessionDuration.min) {
    recommendations.push(
      "Mashq vaqtini uzaytiring yoki intensivligini oshiring",
    );
  }
  ...
  const cardiovascularHealth = Math.min(
    100,
    Math.round((minutesPerWeek / goals.weeklyMinutes) * 100),
  );
  const muscularStrength = Math.min(
    100,
    Math.round(
      (workoutsPerWeek / goals.weeklyWorkouts) * 80 + consistencyScore * 0.2,
    ),
  );
  const flexibility = Math.min(
    100,
    Math.round(
      (favoriteWorkoutTypes.includes("yoga") ? 80 : 40) +
        consistencyScore * 0.2,
    ),
  );
  const mentalHealth = Math.min(
    100,
    Math.round(
      (workoutsPerWeek / goals.weeklyWorkouts) * 90 + consistencyScore * 0.1,
    ),
  );
...
export const addSampleWorkoutData = (
  ...
  for (let i = 0; i < 7; i++) {
    ...
    if (!existing && (i === 1 || i === 3 || i === 5)) {
      ...
      const intensity = ["moderate", "high"][Math.floor(Math.random() * 2)] as
        | "moderate"
        | "high";
...
export const getWeeklyWorkoutSummary = (telegramId: string) => {
  ...
  const totalCalories = weekHistory.reduce(
    (sum, w) => sum + w.caloriesBurned,
    0,
  );

  const workoutProgress = Math.round(
    (totalWorkouts / goals.weeklyWorkouts) * 100,
  );
  ...
  return {
    ...
    isGoalMet:
      totalWorkouts >= goals.weeklyWorkouts &&
      totalMinutes >= goals.weeklyMinutes,
  };
};
