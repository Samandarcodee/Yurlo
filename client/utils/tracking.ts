...
import { getTodaySleep, getSleepInsights, addSampleSleepData } from "./sleepTracking";
...
export const calculateNutritionGoals = (user: UserProfile) => {
  ...
  return {
    ...
    fat: Math.round((dailyCalories * 0.30) / 9),     // 30% of calories from fat (9 cal/g)
...
export const calculateDailyCalories = (user: UserProfile): number => {
  ...
  const multiplier = activityMultipliers[user.activityLevel as keyof typeof activityMultipliers] || 1.55;
...
export const getWeeklyStats = (telegramId: string): WeeklyStats => {
  ...
  const averageCalories = weeklyData.length > 0 ? Math.round(
    weeklyData.reduce((sum, day) => sum + day.caloriesConsumed, 0) / weeklyData.length
  ) : 0;
  ...
  const averageSleep = sleepData.length > 0 
    ? sleepData.reduce((sum, sleep) => sum + sleep.sleepDuration, 0) / sleepData.length
    : 0;
  
  const averageSleepQuality = sleepData.length > 0 
    ? sleepData.reduce((sum, sleep) => sum + sleep.sleepQuality, 0) / sleepData.length
    : 0;
  
  // Calculate sleep consistency (0-100 score)
  const sleepConsistency = sleepData.length >= 2 ? (() => {
    ...
    const avgBedTime = sleepTimes.reduce((sum, t) => sum + t.bedTime, 0) / sleepTimes.length;
    const avgWakeTime = sleepTimes.reduce((sum, t) => sum + t.wakeTime, 0) / sleepTimes.length;
    
    const bedVariance = sleepTimes.reduce((sum, t) => sum + Math.pow(t.bedTime - avgBedTime, 2), 0) / sleepTimes.length;
    const wakeVariance = sleepTimes.reduce((sum, t) => sum + Math.pow(t.wakeTime - avgWakeTime, 2), 0) / sleepTimes.length;
  ...
  const totalWorkouts = weeklyData.reduce(
    (sum, day) => sum + day.activities.filter(a => a.type !== 'walking').length, 0
  );
  
  const totalMeditation = weeklyData.reduce(
    (sum, day) => sum + day.activities
      .filter(a => a.name.toLowerCase().includes('meditation') || a.name.toLowerCase().includes('yoga'))
      .reduce((actSum, act) => actSum + act.duration, 0), 0
  );
  
  const weights = weeklyData.filter(day => day.weight).map(day => day.weight!);
  ...
  const averageCaloriesBurned = weeklyData.reduce((sum, day) => {
    const dailyBurned = day.activities.reduce((actSum, activity) => {
      ...
      return actSum + calculateCaloriesBurned(activity.type, activity.duration, weight);
    }, 0);
    return sum + dailyBurned;
  }, 0) / 7; // average per day
  
  // Calculate average water intake
  const averageWaterIntake = weeklyData.reduce((sum, day) => sum + day.waterIntake, 0) / 7;
