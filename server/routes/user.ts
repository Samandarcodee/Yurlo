...
export const createOrUpdateProfile: RequestHandler = (req, res) => {
  try {
    ...
    const telegramId = req.headers['x-telegram-id']?.toString() || 'demo_user_123';
...
export const updateProfileFields: RequestHandler = (req, res) => {
  try {
    ...
    const allowedFields = [
      'name', 'weight', 'height', 'activityLevel', 'goal', 
      'sleepTime', 'wakeTime', 'language'
    ];
    ...
    if (updatedData.weight || updatedData.height || updatedData.activityLevel) {
      ...
      const activityLevel = updatedData.activityLevel || existingProfile.activityLevel;
      ...
      const dailyCalories = Math.round(bmr * activityMultiplier[activityLevel as keyof typeof activityMultiplier]);
